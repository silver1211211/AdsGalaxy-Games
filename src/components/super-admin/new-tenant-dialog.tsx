...
export function NewTenantDialog(){
  ...
  return <><button type="button" onClick={()=>ref.current?.showModal()} className="game-primary"><Plus size={16}/>New Tenant</button>
    <dialog ref={ref} className="m-auto max-h-[calc(100dvh-2rem)] w-[min(34rem,calc(100%-1rem))] overflow-y-auto rounded-3xl bg-[var(--sa-surface)] p-0 text-[var(--sa-text)] shadow-2xl backdrop:bg-slate-950/60">
      ...
      <TenantUrlPreview slug={normalizeTenantSlug(slug) || "tenant-path"} />
...
function TenantUrlPreview({slug}:{slug:string}){const urls=tenantUrls(slug);return <div className="grid gap-2 rounded-xl bg-[var(--sa-surface-2)] p-3 text-xs"><p className="break-all"><b>Public Mini App URL:</b><br/>{urls.public}</p><p className="break-all"><b>Administrator Login URL:</b><br/>{urls.administratorLogin}</p><p className="break-all"><b>Administrator Dashboard URL:</b><br/>{urls.administratorDashboard}</p></div>}
