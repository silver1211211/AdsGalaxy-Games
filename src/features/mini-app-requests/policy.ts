...
export const requestSchema = z.object({
  ...
  telegramUsername: z.string().trim().max(64).regex(/^@?[A-Za-z0-9_]{5,64}$/).transform(normalizeTelegramUsername).optional().or(z.literal("")),
  ...
  expectedFirstWeekUsers: z.number().int().min(0).max(10_000_000),
  promotionPlan: z.string().trim().min(100).max(2000).refine(meaningful, "Promotion plan must contain meaningful detail"),
  additionalLinks: z.array(url).max(5).transform((links) => [...new Set(links)]).default([]),
