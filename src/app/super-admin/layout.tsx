...
export default async function Layout({ children }: { children: React.ReactNode }) {
  let session;
  try { session = await requireSuperAdminIdentity(); }
  catch { redirect("/dev/access?next=/super-admin"); }
  ...
  const theme = isSuperAdminTheme(session.user.superAdminTheme) ? session.user.superAdminTheme : "LIGHT";
  return <SuperAdminShell initialTheme={theme} accountName={`${session.user.firstName}${session.user.lastName ? ` ${session.user.lastName}` : ""}`}>{children}</SuperAdminShell>;
}
