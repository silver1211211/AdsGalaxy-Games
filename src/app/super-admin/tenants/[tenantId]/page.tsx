...
export default async function TenantDetail({params}:{params:Promise<{tenantId:string}>}){
  ...
  return <div className="grid gap-6"><header><Link href="/super-admin/tenants" className="sa-muted text-xs font-bold">Tenants /</Link><h1 className="mt-2 text-3xl font-black">{t.name}</h1><p className="sa-muted mt-1 text-sm">{t.slug} · {t.status} · Created {t.createdAt.toLocaleDateString()}</p></header>
    ...
    <section className="grid gap-4 lg:grid-cols-2"><article className="sa-card p-5"><h2 className="font-black">Tenant URLs</h2><a className="mt-3 block break-all text-xs font-bold text-teal-600" href={urls.public}>Public Mini App URL<br/>{urls.public}</a><a className="mt-2 block break-all text-xs font-bold text-teal-600" href={urls.administratorLogin}>Administrator Login URL<br/>{urls.administratorLogin}</a><a className="mt-2 block break-all text-xs font-bold text-teal-600" href={urls.administratorDashboard}>Administrator Dashboard URL<br/>{urls.administratorDashboard}</a></article>
