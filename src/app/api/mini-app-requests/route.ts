...
import { hashDeviceIdentifier, REQUEST_DEVICE_COOKIE } from "@/features/mini-app-requests/device";
export async function POST(request:Request){
  try{assertSameOrigin(request);const auth=await requireSession();if(auth.source==="DEVELOPMENT")return Response.json({error:"A verified Telegram session is required.",code:"TELEGRAM_SESSION_REQUIRED"},{status:403});rateLimit(`mini-app-request:${auth.userId}`,3,60*60_000);
    const rawDevice=(await cookies()).get(REQUEST_DEVICE_COOKIE)?.value;if(!rawDevice)return Response.json({error:"Secure device verification is required. Refresh and try again.",code:"DEVICE_REQUIRED"},{status:428});
    const result=await submitMiniAppRequest(auth.userId,hashDeviceIdentifier(rawDevice),await request.json());
    ...
    const message=error instanceof Error?error.message:"";const map:Record<string,string>={INVALID_SLUG:"Choose a valid available Mini App path.",ACTIVE_REQUEST_EXISTS:"This Telegram account or device already has an active Mini App request.",SLUG_UNAVAILABLE:"That Mini App path is unavailable.",APPLICANT_UNAVAILABLE:"Your account cannot submit a request."};
