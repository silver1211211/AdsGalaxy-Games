...
export default async function TenantAdministratorVerification({
  params,
}: {
  ...
  try {
    ...
    redirect(
      process.env.NODE_ENV === "development"
        ? `/dev/access?next=/${tenantSlug}/administrator-verification`
        : "/",
    );
  }
  ...
  if (elevation.ok)
    redirect(
      elevation.mustChangePassword
        ? `/${tenantSlug}/administrator-security`
        : `/${tenantSlug}/admin`,
    );
  return (
    <AdministratorPasswordForm
      endpoint={`/api/${tenantSlug}/admin/security/verify`}
      nextPath={`/${tenantSlug}/admin`}
    />
  );
}
